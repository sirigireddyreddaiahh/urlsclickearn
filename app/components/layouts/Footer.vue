...
<template>
  <footer
    class="relative bg-gradient-to-t from-slate-100 to-white dark:from-slate-900 dark:to-slate-800 border-t border-slate-200/50 dark:border-slate-700/50">
    ...
    <section class="md:pt-8">
      <div class="container flex flex-col items-center py-8 mx-auto sm:flex-row max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex-shrink-0 mb-6 sm:mb-0">
          <NuxtLink to="https://urlsclickearn.xyz"
            ...
            <div class="relative">
              <div
                class="w-10 h-10 brand-gradient rounded-xl flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all duration-300">
                <img :src="logo" :alt="title" class="w-8 h-8 rounded-lg" loading="lazy" />
              </div>
              <!-- Animated ring on hover -->
              <div
                class="absolute inset-0 rounded-xl ring-2 ring-blue-500/20 group-hover:ring-blue-500/40 transition-all duration-300 opacity-0 group-hover:opacity-100" />
            </div>
        ...
        <!-- Copyright Section -->
        <div
          class="flex-1 text-center sm:text-left sm:ml-6 sm:pl-6 sm:border-l sm:border-slate-200/50 dark:sm:border-slate-700/50">
          <a href="https://html.zone" target="_blank" rel="noopener noreferrer"
        ...
        <div class="flex items-center justify-center mt-6 sm:mt-0 sm:ml-auto">
          <div class="flex space-x-4">
            <template v-for="link in socialLinks" :key="link.title">
              <a :href="link.href" :title="link.title" :target="link.external ? '_blank' : undefined"
                ...
                <component :is="link.icon" class="w-6 h-6 transition-all duration-300 group-hover:drop-shadow-lg" />

                <!-- Hover effect background -->
                <div
                  class="absolute inset-0 rounded-full bg-current opacity-0 group-hover:opacity-10 transition-opacity duration-300 transform scale-0 group-hover:scale-100" />

                <!-- Ripple effect on click -->
                <div
                  class="absolute inset-0 rounded-full bg-current opacity-0 group-active:opacity-20 transition-opacity duration-150 transform scale-0 group-active:scale-125" />
              </a>
            </template>
          </div>
        </div>
      ...
      <div class="border-t border-slate-200/50 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-900/50">
        <div class="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div
            class="py-4 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 dark:text-slate-400">
            <div class="mb-2 sm:mb-0">Built with ❤️ using Nuxt.js, Vue.js & Tailwind CSS</div>
            <div class="flex space-x-4">
              <NuxtLink to="/privacy"
                class="hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-300">
                Privacy
              </NuxtLink>
              <NuxtLink to="/terms" class="hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-300">
                Terms
              </NuxtLink>
              <NuxtLink to="/contact"
                class="hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-300">
