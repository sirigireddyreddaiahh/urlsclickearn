...
function formatName(name, type) {
  if (!name || typeof Intl === 'undefined') return name;
  ...
  <TooltipProvider>
    <Tooltip>
      <TooltipTrigger class="w-full text-left">
        <DashboardAnalysisMetricsNameReferer v-if="name && type === 'referer'" :name="name" />
        <DashboardAnalysisMetricsNameSlug v-else-if="name && type === 'slug'" :name="name" />
        <DashboardAnalysisMetricsNameIcon
          v-else-if="
            name && ['os', 'browser', 'browserType', 'device', 'deviceType'].includes(type)
          "
          :name="name"
          :type="type"
        />
