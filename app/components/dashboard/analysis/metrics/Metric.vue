...
<template>
  <Card class="flex flex-col">
    <template v-if="metrics.length">
      <DashboardAnalysisMetricsList class="flex-1" :metrics="top6" :type="type" />
      <CardFooter class="py-2">
        <Dialog>
          ...
          <DialogContent
            class="max-w-[95svw] max-h-[95svh] md:max-w-screen-md grid-rows-[auto_minmax(0,1fr)_auto]"
          >
            <DialogHeader>
              <DialogTitle>{{ name }}</DialogTitle>
            </DialogHeader>
            <DashboardAnalysisMetricsList class="overflow-y-auto" :metrics="metrics" :type="type" />
          </DialogContent>
        </Dialog>
      </CardFooter>
    </template>
