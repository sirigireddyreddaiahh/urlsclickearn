...
function trafficEvent({ props }, { delay = 0 }) {
  ...
  console.info(
    `from ${props.item.city}(${arc.startLat}, ${arc.startLng}) to ${props.item.COLO}(${arc.endLat}, ${arc.endLng})`
  );
...
function initGlobe() {
  ...
  globe.controls().addEventListener(
    ...
    debounce(() => {
      ...
      if (nextAlt !== hexAltitude.value) hexAltitude.value = nextAlt;
    }, 200)
  );
...
onMounted(async () => {
  await Promise.all([getLiveLocations(), getCurrentLocation(), getGlobeJSON(), getColosJSON()]);
  initGlobe();
});
