...
<template>
  <Card class="md:w-80 h-72 flex flex-col p-4 md:m-2">
    <div class="h-24">
      <CardHeader
        v-if="stats.visits"
        class="flex flex-row justify-between items-center pb-2 space-y-0 px-0 pt-2"
      >
      ...
      <CardContent class="px-0 pb-4">
        <NumberFlow
          ...
          :class="{ 'blur-md opacity-60': !stats.visits }"
          :value="stats.visits"
        />
      </CardContent>
    </div>
    <DashboardAnalysisViews class="w-full h-40 border-none !p-0" mode="simple" chart-type="bar" />
  </Card>
</template>
