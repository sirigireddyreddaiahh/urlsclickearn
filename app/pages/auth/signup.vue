      ...
      <div class="bg-white rounded-xl shadow-2xl overflow-hidden">
        ...
        <form class="p-8 space-y-6" @submit.prevent="submit">
          ...
          <div>
            ...
            <div class="mt-3 space-y-2">
              <div class="flex items-center space-x-2">
                <div class="w-3 h-3 rounded-full transition-colors" :class="[password.length >= 8 ? 'bg-green-500' : 'bg-gray-300']" />
                <span class="text-sm" :class="[password.length >= 8 ? 'text-green-600 font-medium' : 'text-gray-500']">
                  At least 8 characters
                </span>
              </div>
              <div class="flex items-center space-x-2">
                <div class="w-3 h-3 rounded-full transition-colors" :class="[/[A-Z]/.test(password) ? 'bg-green-500' : 'bg-gray-300']" />
                <span class="text-sm" :class="[/[A-Z]/.test(password) ? 'text-green-600 font-medium' : 'text-gray-500']">
              ...
              <div class="flex items-center space-x-2">
                <div class="w-3 h-3 rounded-full transition-colors" :class="[/[0-9]/.test(password) ? 'bg-green-500' : 'bg-gray-300']" />
                <span class="text-sm" :class="[/[0-9]/.test(password) ? 'text-green-600 font-medium' : 'text-gray-500']">
                  One number
                </span>
              </div>
              <div class="flex items-center space-x-2">
                <div class="w-3 h-3 rounded-full transition-colors" :class="[hasSpecialChar ? 'bg-green-500' : 'bg-gray-300']" />
          ...
          <div class="flex items-start">
            ...
            <label for="terms" class="ml-2 text-sm text-gray-600">
              I agree to the <a href="#" class="text-[#FF6B35] hover:text-[#F72B7E] font-medium">Terms of Service</a> and
              <a href="#" class="text-[#FF6B35] hover:text-[#F72B7E] font-medium">Privacy Policy</a>
            </label>
          </div>
          ...
          <div
            v-if="msg" class="p-4 rounded-lg text-sm font-medium" :class="[
              msg.includes('successful') ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200',
            ]"
          >
            {{ msg }}
          </div>
        ...
        <div class="px-8 pb-8">
          ...
          <div class="mt-6 grid grid-cols-1 gap-3">
            <a href="/api/auth/oauth/google" class="flex items-center justify-center py-2.5 px-4 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
